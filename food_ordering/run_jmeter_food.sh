#!/usr/bin/env bash
# Non-GUI run for food ordering test plan
# Usage: ./run_jmeter_food.sh [--threads N] [--ramp R] [--loops L] [--help]

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PLAN="$SCRIPT_DIR/food_test_plan.jmx"
JTL="$SCRIPT_DIR/food_non_gui_result.jtl"
REPORT_DIR="$SCRIPT_DIR/food_report"

# prefer system jmeter, else fallback to bundled one under repo root
if command -v jmeter >/dev/null 2>&1; then
  JMETER_CMD="jmeter"
else
  JMETER_CMD="$REPO_ROOT/apache-jmeter-5.6.3/bin/jmeter"
fi
...
echo "Waiting for app at http://localhost:5002/ to be reachable..."
if ! wait_for_url "http://localhost:5002/" 20; then
...
Runs the food ordering JMeter test plan in non-GUI mode and generates an HTML report.
...
echo "Running Food test plan: threads=$THREADS ramp=$RAMP loops=$LOOPS"
echo "Running Food test plan: threads=$THREADS ramp=$RAMP loops=$LOOPS"

# helper: wait for http URL to respond
wait_for_url() {
  local url="$1"
  local timeout=${2:-15}
  local interval=1
...
if ! wait_for_url "http://localhost:5002/" 15; then
  echo "Timeout waiting for http://localhost:5002/ to become available" >&2
  exit 3
fi
